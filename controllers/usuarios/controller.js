...
export const getUsuarios = async () => {
  ...
  export const addUsuario = async (nombre, correo, contrasena) => {
    try {
      const result = await pool.query("INSERT INTO usuarios (nombre, correo, contrasena) VALUES (?, ?, ?)", [nombre, correo, contrasena]);
      return result;
    } catch (error) {
  ...
  export const updateUsuario = async (id, newNombre, newCorreo, newContrasena) => {
    try {
      const result = await pool.query("UPDATE usuarios SET nombre = ?, correo = ?, contrasena = ? WHERE id = ?", [newNombre, newCorreo, newContrasena, id]);
      return result;
    } catch (error) {
